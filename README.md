...
安装前确保系统已经安装了 `PHP`、`Mysql`、`Composer`，`Apache 或 Nginx`。 

推荐使用命令行安装，因为采用命令行安装的方式可以和勾股OA随时保持更新同步,使用命令行安装请提前准备好`Git`，支持**Linux**、**Windows**系统安装的。
...
没有git的，可以点击右上角的【克隆/下载】进行打包下载。下面的安装步骤以代码所在目录“D:\guoguoa”目录为例子。
...
第四步：添加虚拟主机并绑定到项目里面的`public`目录 ，实际部署中，确保绑定域名访问到的是`public`目录。**（这一步很重要，很多人出错）**
